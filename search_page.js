...
function formatStayPeriod(checkIn, checkOut) {
  ...
  const yy1 = String(inDate.getFullYear()).slice(-2);
  const mm1 = String(inDate.getMonth() + 1).padStart(2, "0");
  const dd1 = String(inDate.getDate()).padStart(2, "0");

  const yy2 = String(outDate.getFullYear()).slice(-2);
  const mm2 = String(outDate.getMonth() + 1).padStart(2, "0");
  const dd2 = String(outDate.getDate()).padStart(2, "0");

  return `${yy1}/${mm1}/${dd1} ~ ${yy2}/${mm2}/${dd2}(${nights}泊)`;
}
...
function fillFormWithData(data) {
  ...
  if (stayDisplay) {
    stayDisplay.value = formatStayPeriod(data.checkIn, data.checkOut);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    // 🔥 해시 없이 QR 텍스트 생성
    const qrText = `${room},${checkIn},${checkOut},${guests},${reservation},${breakfastFlag}`;

    // (QR code popup logic removed)
  });
